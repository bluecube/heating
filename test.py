...
for data in open('packets.txt', 'r'):
    ...
    p = db_net.udp.Packet.from_bytes(data)#, 48414)
    if p.mode == db_net.udp.Packet.INVALID_STATION_KEY:
    ...
    try:
        rq = db_net.registers.ReadRequest.from_bytes(p.dbnet_packet.payload)
    except:
        pass
    else:
        print(rq)
    ...
    resp = db_net.registers.ReadResponse.from_bytes(p.dbnet_packet.payload, rq)
    try:
        pass
